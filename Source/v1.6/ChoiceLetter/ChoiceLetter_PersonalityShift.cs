...
namespace ArtificialBeings
{
    public class ChoiceLetter_PersonalityShift : ChoiceLetter_GrowthMoment
    {
        ...
        {
            text = "ABF_PersonalityShift".Translate(pawn);
            mouseoverText = text;
            if (traitChoiceCount > 0 || passionGainsCount > 0)
            {
                mouseoverText += "\n\n" + "ABF_PersonalityShiftChooseHowPawnWillChange".Translate(pawn);
            }
        }
        ...
        {
            base.ExposeData();
            Scribe_Values.Look(ref isReplacingTrait, "ABF_isReplacingTrait", false);
            Scribe_Values.Look(ref isReplacingPassions, "ABF_isReplacingPassions", false);
        }
    }
}