...
namespace ArtificialBeings
{
    ...
    public class JobGiver_AIExterminatorSap : ThinkNode_JobGiver
    {
        protected override Job TryGiveJob(Pawn pawn)
        {
            ...
            if (target != null)
            {
                using (PawnPath pawnPath = pawn.Map.pathFinder.FindPathNow(pawn.Position, target.Position, TraverseParms.For(pawn, Danger.Deadly, TraverseMode.PassAllDestroyableThings)))
                {
                    if (!pawnPath.Found)
                    {
                        return null;
